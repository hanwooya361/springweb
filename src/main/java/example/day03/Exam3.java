...
class SampleController1{
    void method(){
        ...
        sampleDao.method(); // 해당 인스턴스가 메소드 호출
    }
}
...
@Component
class SampleDao3{
    // 싱글톤 대신에 @Component 이용해 IOC(자동 객체 관리) 규칙으로 스프링 컨테이너에 자동 빈(객체)등록
    void method(){} 
}
class SampleController3{
    @Autowired  // 1. 의존성 주입, 스프링 컨테이너(메모리)에 등록된 빈(깩체) 가져와서 대입
...
/*
    다른 클래스를 호출하기 위해선 객체가 필요
    1. [전통] new 클래스명().메소드명();
    2. [미니프로젝트2(싱글톤)] 클래스명().getInstance().메소드명();
    3. [스프링] @Component 등록, @Autowired 호출
    ------------ 
    @Component 등록되는 시점: @SpringBootApplication(@ComponentScan)
    즉] 스프링이 켜지면서 폴더내 @Component 다 찾아(같은폴더/하위폴더) 내에서 스프링 컨테이너 등록 구조
    AppStart 주의할점: 항상 최상위에 위치 
*/